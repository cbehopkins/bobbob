...
// isAllocatorEmpty checks if an allocator has no allocated blocks.
// Returns true if all slots in the allocator are free.
func (p *allocatorPool) isAllocatorEmpty(ref *allocatorRef) bool {
	if ref == nil || ref.allocator == nil {
		return true
	}
	for _, allocated := range ref.allocator.allocatedList {
		if allocated {
			return false
		}
	}
...
// If an allocator becomes completely empty after freeing AND has a smaller blockCount
// than the current pool configuration, it is deleted (as it's sub-optimal and likely
// from an older pool state).
	...
	for i, ref := range p.available {
		...
		if err == nil {
			...
			if p.isAllocatorEmpty(ref) && ref.allocator.blockCount < p.blockCount {
				...
				// Free the allocator's storage from the parent allocator
				if ref.fileOff >= 1 {
					size := ref.sizeInBytes(ref.allocator.blockCount)
					_ = p.parent.Free(ref.fileOff, size) // Ignore errors; gap remains
				}
			}
			return nil
		}
	...
	for i, ref := range p.full {
		...
		if err == nil {
			...
			if p.isAllocatorEmpty(ref) && ref.allocator.blockCount < p.blockCount {
				...
				// Move to available list if not deleted
